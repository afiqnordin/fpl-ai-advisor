...
def get_my_team():
    ...
    Note: Returns your last saved team, not live my-team view.
    """
    ...
    # ── GET YOUR TEAM PICKS (public endpoint) ──────────
    picks_url = f"{FPL_BASE}/entry/{manager_id}/event/{current_gw}/picks/"
    picks_response = session.get(picks_url)

    if picks_response.status_code != 200:
        # Try previous gameweek if current not available yet
        picks_url = f"{FPL_BASE}/entry/{manager_id}/event/{current_gw - 1}/picks/"
        picks_response = session.get(picks_url)

    picks_data = picks_response.json()

    # ── GET YOUR TRANSFER/BUDGET INFO ──────────────────
    entry_url = f"{FPL_BASE}/entry/{manager_id}/"
    entry_data = session.get(entry_url).json()

    bank = entry_data.get('last_deadline_bank', 0) / 10
    free_transfers = picks_data.get('entry_history', {}).get('event_transfers', 0)

    # Calculate free transfers properly
    transfers_url = f"{FPL_BASE}/entry/{manager_id}/transfers/"
    transfers_data = session.get(transfers_url).json()
    ...
    return {
        ...
        "free_transfers": 1,  # Default — update manually if you have wildcards etc
        "players": my_players
    }


def format_team_for_agent(team_data):
